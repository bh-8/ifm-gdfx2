...
Die besten Werte konnten für das \resnet-Modell mit einer Sequenzlänge von 12 erreicht werden (\resnet-12), was auf einen guten Trade-Off zwischen Performance und Rechenleistung hindeutet.

\section{Zusammenfassung}
\subsection{Fazit}
Im Rahmen dieser Arbeit wurde eine Architektur zur Klassifikation von DeepFake-Bildsequenzen entwickelt.
Die Modelle bestehen aus einem Merkmalsextraktor (\resnet oder \effnet) als Baseline-Klassifikator mit einem dahinterliegenden BiLSTM und unterscheiden die Klassen \textit{original}, \textit{face-swap} und \textit{face-reenactment}.
Dabei konnte gezeigt werden, dass mit \resnet insgesamt bessere Ergebnisse erzielt werden können.
Obwohl die Leistung aller Modelle mit zunehmender Sequenzlänge leicht steigt, scheint bei Sequenzen der Länge 12 ein Sweet-Spot bezogen auf die verwendeten Trainingsparameter erreicht zu sein.
