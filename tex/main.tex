...
df40/
    ...
    \begin{enumerate}
        \item\textbf{Input-Layer}: In der Eingabeschicht wird die Form des Eingabetensors definiert. Diese ergibt sich zum einen aus den Abmessungen eines einzelnen Frames (\texttt{224x224x3}), zum anderen aus der Länge der Sequenz (\texttt{8}, \texttt{12} oder \texttt{16}), mit der das Modell arbeiten soll.
        \item\textbf{FeatureExtraction-Layer}: Diese Schicht implementiert eine Merkmalsextraktion für jeden Frame in der Sequenz (\enquote{\texttt{TimeDistributed}}), wahlweise durch ResNet50 oder EfficientNetB0. Die Gewichte dieser Schicht werden initial mit vortrainierten Gewichten via \enquote{\texttt{imagenet}}~\footnote{ImageNet, \url{https://www.image-net.org/}} belegt.
    \end{enumerate}
    \begin{itemize}
